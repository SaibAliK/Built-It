...
// );
        ...
        $perPage = round($totalPages / $pages,1);
        $page = intval(self::$_draw);
        if (intval(self::$_draw) * $perPage > intval($totalPages) && intval(self::$_draw) > 1) {
            if ((intval(self::$_draw) - 1) * $perPage < intval($totalPages)) {
                self::$_query->take(self::$_length)->skip(self::$_start);
            } else {
                self::$_query->take(self::$_length);
                $page = 1;
                self::$_start = 1;
            }
